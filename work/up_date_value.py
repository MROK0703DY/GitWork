...
class UpdateValue:
    ...
    def multiple_substitution_list(self, a_list: list, a_dict: dict) -> list:
        """Метод заменяет несколько элементов в списке. Список не меняется."""
...
def main() -> None:
    ...
    p()
    up = UpdateValue()
    pp(up.update_inner_value(player, ["localize", "en", "nickname"], 8080))
    p()
    pp(up.update_inner_value(player, ["status"], 4))
